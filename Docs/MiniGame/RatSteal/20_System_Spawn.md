...
- 밭별 최대 작물 수는 A=20, B=15, C=10을 유지한다.
- 시작 직후 빈 맵처럼 보이지 않도록 BeginPlay에서 초기 작물을 먼저 배치한다.
- 기본 초기 배치 수는 A=8, B=6, C=4이며 이후 기존 스폰 간격으로 최대치까지 보충된다.
- 초기 배치도 기존 스폰 규칙과 같은 영역 제외, 거리 검사, 작물 확률, 단계 스프라이트 검증을 통과해야 생성된다.
