...
- 공격 지시자 패턴은 원작 `AttackPattern`의 offset 방식으로 생성한다.
- 현재 패턴은 세로 3점, 가로 3점, 십자 5점, 대각 5점, 대각선 3점 중 랜덤 선택한다.
- 공격 실행 후 `AttackFlipbook`은 비루프로 처음부터 재생하고, 실제 flipbook 총 길이 이상 동안 이동/상태 복귀를 막는다.
- 공격 애니메이션이 끝난 뒤에만 플레이어가 공격 범위 안이면 다음 공격 대기, 범위 밖이면 추적으로 전환한다.
