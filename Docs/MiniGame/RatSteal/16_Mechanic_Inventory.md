...
- 입력은 원작 그대로 `Z=훔치기`, `X=슬롯 전환`, `C=버리기`를 사용한다.
- 슬롯은 작물별 고정 슬롯이다. Slot1=가지, Slot2=감자, Slot3=호박.
- 훔친 작물은 고정 슬롯의 카운트에만 누적된다. 빈 슬롯에 다른 작물이 들어가는 방식은 사용하지 않는다.
- `C`로 버리면 현재 선택 슬롯의 카운트가 1 감소하고, 감속 배수가 즉시 낮아져 기본 이동 속도가 회복된다.
- UE 구현에서는 버리기 성공 시 짧은 대시 보정과 포물선 투척 비주얼을 함께 재생한다.
