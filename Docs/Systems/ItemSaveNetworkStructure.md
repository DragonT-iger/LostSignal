...
위 세 가지는 현재 구현 상태의 전제이며, **출시 방침도 이 구조를 유지한다**(로컬 `SaveGame` + Steam Cloud). 서버 권위로 옮기는 전환은 학습 목적의 별도 주제이고 [SaveBackendMigration.md](SaveBackendMigration.md)가 단일 출처다. 이 문서는 계획이 아니라 현재 코드를 기술한다.
...
여기서 다루는 것은 **저장 데이터 관점의 멀티 전환**이다. 데디케이티드 서버 프로세스 자체를 만드는 작업(빌드 타깃·서버 부팅·접속/이탈·전역 상태 정리·운영)은 [DedicatedServerBuildout.md](DedicatedServerBuildout.md)가 단일 출처다.
