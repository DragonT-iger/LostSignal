...
`DT_MonsterStat`, `DT_MonsterAction`, 캐릭터 소음 프로필 테이블은 `ULSGameDataSettings`에 한 번만 등록하고 `ULSGameDataSubsystem`이 로드·조회한다. 몬스터 BP와 컴포넌트는 DataTable 에셋을 직접 소유하지 않으며 `MonsterRowName`과 `Action_Group`의 row 이름만 보관한다.

몬스터 공용 텔레그래프 Circle/Box 머티리얼, 적 체력바 위젯 클래스, 시야 잔상 머티리얼은 `ULSMonsterPresentationSettings`가 단일 참조를 소유한다. 몬스터 BP의 `ULSMonsterCombatComponent`, `ULSEnemyHealthBarComponent`, `ULSVisionGhostComponent`에 같은 에셋을 반복 할당하지 않는다.
