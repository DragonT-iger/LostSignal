...
- `ULSSurvivalStatusWidget`은 `ChipImage`의 `M_UI_CircleIcon` UI 머티리얼 `IconTexture` 파라미터에 현재 신호 게이지 10% 구간에서 다음으로 비활성화될 하드웨어 슬롯의 칩 아이콘을 출력한다. `SurvivalCooldownRingImage`의 `Progress`는 해당 칩이 비활성화 임계값까지 남은 구간 비율(1.0→0.0)을 표시하며, 슬롯이 비어 있거나 신호가 0%라면 칩 이미지를 숨긴다.
