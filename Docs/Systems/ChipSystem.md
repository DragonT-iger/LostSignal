...
- `ULSSurvivalStatusWidget`은 `SignalProgressBar1~10`을 왼쪽부터 10등분 신호 유실 바로 사용한다. 전체 신호 게이지(1.0→0.0)를 각 칸의 연속 `Percent`로 분배해 칩이 사라질 때마다 한 칸씩 소진되며, 현재 칸은 드레인 타이머 잔여시간에 따라 부드럽게 감소한다. Fill/Background 브러시와 색상은 `WBP_SurvivalStatus`의 각 ProgressBar 스타일에서 관리한다.
  - `ALSFarmingGameMode`가 신호 드레인을 도는 레벨(실제 레이드/디버그 테스트 맵)에서는 클라이언트 자체 추정이 아니라 드레인 타이머 실제 잔여시간(`GetSignalGaugeDrainRemainingSeconds` / `GetSignalGaugeDrainInterval`)을 매 틱 읽어 현재 10% 구간을 보간한다. 판정 기준은 정식 레이드 세션 플래그가 아니라 **현재 게임모드가 `ALSFarmingGameMode`이고 타이머가 도는지**다. 다른 게임모드(로비/결과)거나 타이머가 멈춘 경우(게이지 0%)에는 로비/프리뷰와 동일하게 저장 게이지 위치를 정적으로 표시한다.
- 현재 생존 프로토콜 레벨이 0(미해금)이면 신호 유실 바(`SignalProgressBar1~10`)를 숨긴다. HP/스태미나 바 게이팅(현재 레벨 ≥ 1)과 동일 기준이며, 칩 스테이션 프리뷰·실제 HUD 공용이다. `WBP_ChipStation` 프리뷰는 슬라이더 값을, 실제 HUD는 저장된 `ChipSignalGaugePercent` 값을 기준으로 갱신한다.
