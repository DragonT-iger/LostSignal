...
- HP는 `ULSCombatAttributeSet`의 `CurrentHealth/MaxHealth`, 스태미나는 `ULSCharacterAttributeSet`의 `CurrentStamina/MaxStamina`를 ASC attribute delegate로 구독해 `현재/최대` 텍스트와 게이지로 갱신한다.
