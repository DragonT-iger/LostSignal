...
- **신호 게이지(Signal Gauge)**: 0~100% 스크롤바. 게이지가 90.0% 이하로 내려갈 때부터 10% 단위로 **칩 번호 순서대로** 비활성화된다. 기본 장착 칩 스탯/프로토콜 합산은 활성·비활성 슬롯을 모두 포함하며, 비활성 칩 스탯 값의 50%는 `SignalLossText`에 별도로 표시한다. 최종 전투 스탯은 기본 스탯 표시값과 `SignalLossText` 표시값을 더한 값이다.
...
4. ~~**신호 게이지 비활성 경계**~~: **90.0% 이하부터 1번 슬롯 비활성, 이후 10% 단위로 2~10번 슬롯을 순서대로 비활성화한다.** 기본 장착 칩 스탯/프로토콜 합산은 활성·비활성 칩을 모두 포함하고, 비활성 칩 스탯 값의 50%를 `SignalLossText`에 표시한다. 최종 전투 스탯은 기본 표시값과 `SignalLossText` 표시값을 합산한다.
