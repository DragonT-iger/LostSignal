...
void ULSChipStationWidget::RefreshEquippedChipSummary()
{
	...
	SetPreviewSignalProgress(GetSignalGaugePercent());
...
void ULSChipStationWidget::SetPreviewSignalProgress(const float SignalPercent)
{
	if (!SurvivalStatus)
	{
		UE_LOG(LogLS, Warning, TEXT("SurvivalStatus is not bound on %s."), *GetNameSafe(this));
	...
	SurvivalStatus->SetPreviewSignalProgress(SignalPercent);
}
