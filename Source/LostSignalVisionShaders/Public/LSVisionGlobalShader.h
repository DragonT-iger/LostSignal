...
public:
	...
	BEGIN_SHADER_PARAMETER_STRUCT(FParameters, )
		...
		SHADER_PARAMETER_RDG_BUFFER_SRV(StructuredBuffer<float>, PolygonPointFlags)
		...
		SHADER_PARAMETER(float, NoiseWidth)
		SHADER_PARAMETER(float, OccluderFeatherScale)
...
namespace LSVision
{
	struct LOSTSIGNALVISIONSHADERS_API FMaskDispatchInputs
	{
		...
		FRDGBufferSRVRef PolygonPointFlagsSRV = nullptr;
		FRHITexture* NoiseTexture = nullptr;
		float NoiseScale = 0.0f;
		float NoiseWidth = 0.0f;
		float OccluderFeatherScale = 0.0f;
	};
